...
int main()
{
	...
	AddRule("192.168.152.1", "any", -1, -1, -1, 1, 1);
	AddRule("any", "192.168.152.1", -1, -1, -1, 1, 1);
	AddRule("192.168.164.1", "any", -1, -1, -1, 1, 1);
	AddRule("any", "192.168.164.1", -1, -1, -1, 1, 1);
	AddRule("192.168.164.0/24", "any", -1, -1, -1, 1,1);
	AddRule("any", "192.168.164.0/24", -1, -1, -1, 1,1);
	AddRule("any", "any", -1, -1, -1, 1, 1);
...
printf("sending\n");
	...
	unsigned short firewall_port = 8888;
	unsigned short nat_port = 80;
	SetNat(ipstr_to_num("192.168.164.0"), 0xffffff00, ipstr_to_num("192.168.152.1"));
	AddNatRule(ipstr_to_num(nat_ip), ipstr_to_num(firewall_ip), nat_port, firewall_port);
	PrintNatRules();
	SendNatRules();
